...
PhysicsCollisionShape* PhysicsController::createMesh(Mesh* mesh, const Vector3& scale, bool dynamic)
{
    ...
    if (dynamic)
    {
        ...
        btConvexHullShape* originalConvexShape = new btConvexHullShape(shapeMeshData->vertexData, data->vertexCount, sizeof(float)*3);
	    ...
	    collisionShape = new btConvexHullShape((btScalar*)hull->getVertexPointer(), hull->numVertices());
    ...
    else
    {
        ...
        collisionShape = new btBvhTriangleMeshShape(meshInterface, true);
    }
